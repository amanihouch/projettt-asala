<!-- frontend/src/views/BecomeVendor.vue - VERSION FINALE CORRIGÉE -->
<template>
  <div class="become-vendor-page" :class="{ 'dark-mode': isDarkMode }">
    <div class="vendor-wrapper">
      <!-- Left Column - Branding -->
      <div class="brand-column">
        <div class="brand-content">
          <img src="/src/assets/asala logo.svg" alt="ASALA" class="brand-logo" />
          <div class="welcome-message">
            <h2 class="welcome-title">انضم إلينا كحرفي</h2>
            <p class="welcome-text">شارك إبداعاتك مع العالم</p>
            <p class="welcome-subtext">انضم إلى مجتمع الحرفيين التونسيين</p>
          </div>
        </div>
      </div>

      <!-- Right Column - Form -->
      <div class="form-column">
        <div class="form-card">
          <!-- Progress Steps -->
          <div class="progress-container">
            <div class="progress-steps">
              <div v-for="(step, idx) in steps" :key="idx" class="step-wrapper">
                <div class="step" :class="{ active: currentStep >= idx + 1, completed: currentStep > idx + 1 }">
                  <div class="step-number">
                    <span v-if="currentStep > idx + 1">✓</span>
                    <span v-else>{{ idx + 1 }}</span>
                  </div>
                  <div class="step-label">{{ step }}</div>
                </div>
                <div v-if="idx < steps.length - 1" class="step-line" :class="{ active: currentStep > idx + 1 }"></div>
              </div>
            </div>
          </div>

          <!-- Step 1: Personal Info -->
          <div v-if="currentStep === 1" class="form-step">
            <div class="step-header">
              <div class="step-badge"><span>👤</span><span>الخطوة 1/3</span></div>
              <h2 class="step-title">المعلومات الشخصية</h2>
              <p class="step-description">أدخل معلوماتك الأساسية</p>
            </div>
            <form @submit.prevent="validateStep1" class="auth-form" autocomplete="off">
              <div class="form-group" :class="{ 'has-error': errors.fullName }">
                <label class="form-label"><span>👤</span>الاسم الكامل<span class="required">*</span></label>
                <input type="text" v-model="form.fullName" class="form-input" autocomplete="off" />
                <span v-if="errors.fullName" class="error-message">{{ errors.fullName }}</span>
              </div>
              <div class="form-group" :class="{ 'has-error': errors.email }">
                <label class="form-label"><span>📧</span>البريد الإلكتروني<span class="required">*</span></label>
                <input type="email" v-model="form.email" class="form-input" dir="ltr" autocomplete="off" />
                <span v-if="errors.email" class="error-message">{{ errors.email }}</span>
              </div>
              <div class="form-group" :class="{ 'has-error': errors.phone }">
                <label class="form-label"><span>📞</span>رقم الهاتف<span class="required">*</span></label>
                <div class="phone-input-wrapper">
                  <span class="country-code">+216</span>
                  <input type="tel" v-model="form.phone" class="form-input phone-field" maxlength="8" autocomplete="off" />
                </div>
                <span v-if="errors.phone" class="error-message">{{ errors.phone }}</span>
              </div>
              <div class="form-group" :class="{ 'has-error': errors.address }">
                <label class="form-label"><span>📍</span>العنوان<span class="required">*</span></label>
                <input type="text" v-model="form.address" class="form-input" autocomplete="off" />
                <span v-if="errors.address" class="error-message">{{ errors.address }}</span>
              </div>
              <div class="form-group" :class="{ 'has-error': errors.password }">
                <label class="form-label"><span>🔒</span>كلمة المرور<span class="required">*</span></label>
                <div class="password-wrapper">
                  <input :type="showPassword ? 'text' : 'password'" v-model="form.password" class="form-input" autocomplete="new-password" />
                  <button type="button" class="toggle-password" @click="showPassword = !showPassword">{{ showPassword ? '🙈' : '👁️' }}</button>
                </div>
                <span v-if="errors.password" class="error-message">{{ errors.password }}</span>
              </div>
              <div class="form-group" :class="{ 'has-error': errors.confirmPassword }">
                <label class="form-label"><span>🔒</span>تأكيد كلمة المرور<span class="required">*</span></label>
                <div class="password-wrapper">
                  <input :type="showConfirmPassword ? 'text' : 'password'" v-model="form.confirmPassword" class="form-input" autocomplete="new-password" />
                  <button type="button" class="toggle-password" @click="showConfirmPassword = !showConfirmPassword">{{ showConfirmPassword ? '🙈' : '👁️' }}</button>
                </div>
                <span v-if="errors.confirmPassword" class="error-message">{{ errors.confirmPassword }}</span>
              </div>
              <div class="form-actions">
                <button type="submit" class="btn-next"><span>التالي</span><span>←</span></button>
              </div>
            </form>
          </div>

          <!-- Step 2: Store Info -->
          <div v-if="currentStep === 2" class="form-step">
            <div class="step-header">
              <div class="step-badge"><span>🏪</span><span>الخطوة 2/3</span></div>
              <h2 class="step-title">معلومات المتجر</h2>
              <p class="step-description">أخبرنا عن متجرك</p>
            </div>
            <form @submit.prevent="validateStep2" class="auth-form">
              <div class="form-group" :class="{ 'has-error': errors.shopName }">
                <label class="form-label"><span>🏷️</span>اسم المتجر<span class="required">*</span></label>
                <input type="text" v-model="form.shopName" class="form-input" />
                <span v-if="errors.shopName" class="error-message">{{ errors.shopName }}</span>
              </div>

              <!-- SPECIALTY + EXPERIENCE IN SAME ROW -->
              <div class="form-row">
                <div class="form-group" :class="{ 'has-error': errors.specialty }">
                  <label class="form-label"><span>🎨</span>التخصص<span class="required">*</span></label>
                  <select v-model="form.specialty" class="form-select">
                    <option value="" disabled>اختر تخصصك</option>
                    <option value="pottery">🏺 فخار</option>
                    <option value="textiles">🧵 منسوجات</option>
                    <option value="jewelry">💍 مجوهرات</option>
                    <option value="woodwork">🪵 خشب</option>
                    <option value="metalwork">⚒️ معادن</option>
                    <option value="leather">👜 جلود</option>
                    <option value="perfumes">🌸 عطور</option>
                    <option value="other">🎨 أخرى</option>
                  </select>
                  <span v-if="errors.specialty" class="error-message">{{ errors.specialty }}</span>
                </div>

                <div class="form-group">
                  <label class="form-label"><span>⏳</span>سنوات الخبرة</label>
                  <input type="number" v-model.number="form.experience" class="form-input experience-input" min="0" max="100" placeholder="0" />
                </div>
              </div>

              <div class="form-group" :class="{ 'has-error': errors.description }">
                <label class="form-label"><span>📝</span>وصف النشاط<span class="required">*</span></label>
                <textarea v-model="form.description" class="form-textarea" rows="4" placeholder="اكتب وصفاً مفصلاً عن نشاطك الحرفي..."></textarea>
                <div class="char-counter" :class="{ warning: form.description.length > 150, danger: form.description.length > 190 }">{{ form.description.length }}/200</div>
                <span v-if="errors.description" class="error-message">{{ errors.description }}</span>
              </div>
              <div class="form-actions">
                <button type="button" class="btn-prev" @click="currentStep = 1"><span>→</span><span>السابق</span></button>
                <button type="submit" class="btn-next"><span>التالي</span><span>←</span></button>
              </div>
            </form>
          </div>

          <!-- Step 3: Photos & Confirmation -->
          <div v-if="currentStep === 3" class="form-step">
            <div class="step-header">
              <div class="step-badge"><span>📸</span><span>الخطوة 3/3</span></div>
              <h2 class="step-title">الصور والتأكيد</h2>
              <p class="step-description">أضف صوراً لمتجرك (اختياري)</p>
            </div>
            <form @submit.prevent="submitForm" class="auth-form">
              <div class="upload-section">
                <label class="form-label"><span>👤</span>الصورة الشخصية<span class="optional">(اختياري)</span></label>
                <div class="upload-area" :class="{ 'has-preview': profilePreview }" @click="triggerProfileUpload">
                  <input type="file" ref="profileInput" @change="handleProfileUpload" accept="image/*" style="display:none" />
                  <div v-if="!profilePreview" class="upload-placeholder">
                    <div class="upload-icon">📸</div>
                    <p class="upload-text">انقر لإضافة الصورة</p>
                    <p class="upload-hint">PNG, JPG - 5MB max</p>
                  </div>
                  <div v-else class="upload-preview">
                    <img :src="profilePreview" alt="Profile" />
                    <button type="button" class="change-photo-btn" @click.stop="triggerProfileUpload">✏️ تغيير</button>
                  </div>
                </div>
              </div>
              <div class="upload-section">
                <label class="form-label"><span>🖼️</span>صورة الغلاف<span class="optional">(اختياري)</span></label>
                <div class="upload-area" :class="{ 'has-preview': coverPreview }" @click="triggerCoverUpload">
                  <input type="file" ref="coverInput" @change="handleCoverUpload" accept="image/*" style="display:none" />
                  <div v-if="!coverPreview" class="upload-placeholder">
                    <div class="upload-icon">🌄</div>
                    <p class="upload-text">انقر لإضافة الغلاف</p>
                    <p class="upload-hint">PNG, JPG - 5MB max</p>
                  </div>
                  <div v-else class="upload-preview cover-preview" :style="{ backgroundImage: `url(${coverPreview})` }">
                    <button type="button" class="change-photo-btn" @click.stop="triggerCoverUpload">✏️ تغيير</button>
                  </div>
                </div>
              </div>
              <div class="summary-grid">
                <div class="summary-card">
                  <div class="summary-header"><span>👤</span><h3>معلومات الحساب</h3></div>
                  <div class="summary-content">
                    <div class="summary-item"><span>الاسم:</span><span>{{ form.fullName || '—' }}</span></div>
                    <div class="summary-item"><span>البريد:</span><span>{{ form.email || '—' }}</span></div>
                    <div class="summary-item"><span>الهاتف:</span><span>{{ form.phone || '—' }}</span></div>
                    <div class="summary-item"><span>العنوان:</span><span>{{ truncateText(form.address, 25) || '—' }}</span></div>
                  </div>
                </div>
                <div class="summary-card">
                  <div class="summary-header"><span>🏪</span><h3>معلومات المتجر</h3></div>
                  <div class="summary-content">
                    <div class="summary-item"><span>اسم المتجر:</span><span>{{ form.shopName || '—' }}</span></div>
                    <div class="summary-item"><span>التخصص:</span><span>{{ getSpecialtyName(form.specialty) || '—' }}</span></div>
                    <div class="summary-item"><span>الخبرة:</span><span>{{ form.experience || 0 }} سنوات</span></div>
                    <div class="summary-item"><span>الوصف:</span><span>{{ truncateText(form.description, 35) || '—' }}</span></div>
                  </div>
                </div>
              </div>
              <div class="terms-section">
                <label class="checkbox-label">
                  <input type="checkbox" v-model="form.acceptTerms" />
                  <span class="checkmark"></span>
                  <span class="checkbox-text">أوافق على <a href="#" class="terms-link">الشروط والأحكام</a><span class="required">*</span></span>
                </label>
              </div>
              <div class="form-actions">
                <button type="button" class="btn-prev" @click="currentStep = 2"><span>→</span><span>السابق</span></button>
                <button type="submit" class="btn-submit" :disabled="isSubmitting">
                  <span v-if="!isSubmitting">تأكيد التسجيل</span>
                  <div v-else class="loader"></div>
                </button>
              </div>
            </form>
          </div>
        </div>
        <div class="login-link">
          <p>لديك حساب بالفعل؟</p>
          <router-link to="/login" class="login-link-btn"><span>تسجيل الدخول</span><span>←</span></router-link>
        </div>
      </div>
    </div>

    <!-- Modal Vérification Email -->
    <transition name="modal">
      <div v-if="showVerificationModal" class="success-popup-overlay">
        <div class="success-popup" :class="{ 'dark-mode': isDarkMode }" dir="rtl">
          <div class="popup-icon-container">
            <div class="icon-circle-success" style="background: linear-gradient(135deg, #08717f, #065a69);">
              <span style="font-size: 2rem;">📧</span>
            </div>
          </div>
          <h2 class="popup-title">تأكيد البريد الإلكتروني</h2>
          <p class="popup-subtitle">تم إرسال رمز التحقق إلى <strong>{{ savedEmailForVerification }}</strong></p>
          <div class="code-inputs" dir="ltr" style="display: flex; gap: 10px; justify-content: center; margin: 20px 0;">
            <input v-for="(_, index) in 6" :key="index" type="text" maxlength="1" inputmode="numeric" pattern="[0-9]*" class="code-input-verify" v-model="verificationDigits[index]" @input="handleCodeInput($event, index)" @keydown.delete="handleCodeDelete($event, index)" @paste="handleCodePaste" dir="ltr" autocomplete="off" />
          </div>
          <div style="display: flex; gap: 10px; flex-direction: column;">
            <button @click="verifyVendorEmail" :disabled="verifyingEmail" class="popup-home-btn">
              <span v-if="!verifyingEmail">تأكيد</span>
              <span v-else>جاري التحقق...</span>
            </button>
            <button @click="resendVendorCode" :disabled="resendingCode" style="background: #f1f5f9; color: #1e293b; border: none; padding: 12px; border-radius: 14px; cursor: pointer;">
              {{ resendingCode ? 'جاري الإرسال...' : 'إعادة إرسال الرمز' }}
            </button>
          </div>
        </div>
      </div>
    </transition>

    <!-- Modal Succès -->
    <transition name="modal">
      <div v-if="showSuccessModal" class="success-popup-overlay" @click.self="closeSuccessModal">
        <div class="success-popup" :class="{ 'dark-mode': isDarkMode }" dir="rtl">
          <button class="popup-close-btn" @click="closeSuccessModal">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
          <div class="popup-logo-container"><img src="/src/assets/asala logo.svg" alt="أصالة" class="popup-logo-img" /></div>
          <div class="popup-icon-container">
            <div class="icon-circle-success">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
            </div>
          </div>
          <h2 class="popup-title">تم إنشاء الحساب بنجاح</h2>
          <p class="popup-subtitle">طلبك قيد المراجعة</p>
          <div class="popup-message-box"><span class="message-emoji">⏳</span><p>نرجو منكم التكرّم بالانتظار ريثما تقوم الإدارة بمراجعة طلبكم والموافقة عليه في غضون <strong class="text-gold">24 ساعة</strong>.</p></div>
          <div class="popup-email-box">
            <img src="/src/assets/img12.jpeg" alt="Success" class="email-image" @error="(e) => e.target.style.display = 'none'" />
            <div class="email-content">
              <div class="email-icon-row"><span class="email-emoji">📧</span><p class="email-text">سيتم إشعاركم عبر البريد الإلكتروني</p></div>
              <strong class="email-highlight">{{ savedEmailForVerification }}</strong>
              <p class="email-subtext">فور إتمام عملية التفعيل</p>
            </div>
          </div>
          <div class="popup-contact-box">
            <div class="contact-row"><span>📧</span><span>للتواصل مع الإدارة</span></div>
            <a href="mailto:infinty.tunisia@gmail.com" class="contact-link">infinty.tunisia@gmail.com</a>
            <p class="contact-hint">في حال عدم تلقيكم لأي رد، يُرجى التواصل معنا عبر البريد الإلكتروني أعلاه.</p>
          </div>
          <button @click="closeSuccessModal" class="popup-home-btn"><span>🏠</span><span>العودة إلى الصفحة الرئيسية</span></button>
        </div>
      </div>
    </transition>

    <!-- Toast -->
    <transition name="toast">
      <div v-if="toast.show" class="toast-notification" :class="[toast.type, { 'dark-mode': isDarkMode }]">
        <div class="toast-content"><span>{{ toast.icon }}</span><span>{{ toast.message }}</span></div>
        <div class="toast-progress"></div>
      </div>
    </transition>

    <!-- Loading -->
    <transition name="fade">
      <div v-if="isLoading" class="loading-overlay"><div class="spinner"></div><p>جاري المعالجة...</p></div>
    </transition>
  </div>
</template>

<script setup>
import { ref, reactive, computed, onMounted } from 'vue'
import { useRouter } from 'vue-router'
import { useAuthStore } from '../stores/auth'
import { useThemeStore } from '../stores/theme'
import api from '../services/api'

const router = useRouter()
const authStore = useAuthStore()
const themeStore = useThemeStore()

const isDarkMode = computed(() => themeStore.isDarkMode)

// ===== STATE =====
const currentStep = ref(1)
const isSubmitting = ref(false)
const isLoading = ref(false)
const errors = ref({})
const showPassword = ref(false)
const showConfirmPassword = ref(false)
const showSuccessModal = ref(false)
const showVerificationModal = ref(false)
const steps = ['المعلومات الشخصية', 'معلومات المتجر', 'الصور والتأكيد']

// ✅ EMAIL SAUVEGARDÉ pour la vérification
const savedEmailForVerification = ref('')

// Vérification email
const verificationDigits = ref(['', '', '', '', '', ''])
const verifyingEmail = ref(false)
const resendingCode = ref(false)

// File handlers
const profileInput = ref(null)
const coverInput = ref(null)
const profilePreview = ref(null)
const coverPreview = ref(null)
const profileFile = ref(null)
const coverFile = ref(null)

// Toast
const toast = ref({ show: false, message: '', type: 'success', icon: '✅' })

// Form data
const form = reactive({
  fullName: '',
  email: '',
  phone: '',
  address: '',
  password: '',
  confirmPassword: '',
  shopName: '',
  specialty: '',
  experience: 0,
  description: '',
  acceptTerms: false
})

// ===== UTILS =====
const truncateText = (text, length) => text?.length > length ? text.substring(0, length) + '...' : text || '—'

const getSpecialtyName = (specialty) => {
  const specialties = { pottery: '🏺 فخار', textiles: '🧵 منسوجات', jewelry: '💍 مجوهرات', woodwork: '🪵 خشب', metalwork: '⚒️ معادن', leather: '👜 جلود', perfumes: '🌸 عطور', other: '🎨 أخرى' }
  return specialties[specialty] || specialty
}

const showNotification = (message, type = 'success') => {
  const icons = { success: '✅', error: '❌', info: 'ℹ️', warning: '⚠️' }
  toast.value = { show: true, message, type, icon: icons[type] }
  setTimeout(() => toast.value.show = false, 3000)
}

// ===== RESET FORM =====
const resetForm = () => {
  form.fullName = ''
  form.email = ''
  form.phone = ''
  form.address = ''
  form.password = ''
  form.confirmPassword = ''
  form.shopName = ''
  form.specialty = ''
  form.experience = 0
  form.description = ''
  form.acceptTerms = false
  profileFile.value = null
  coverFile.value = null
  profilePreview.value = null
  coverPreview.value = null
  verificationDigits.value = ['', '', '', '', '', '']
  showVerificationModal.value = false
  showSuccessModal.value = false
  currentStep.value = 1
  errors.value = {}
  savedEmailForVerification.value = ''
}

// ===== UPLOAD HANDLERS =====
const createPreview = (file) => {
  return new Promise((resolve, reject) => {
    const reader = new FileReader()
    reader.onload = (e) => resolve(e.target.result)
    reader.onerror = reject
    reader.readAsDataURL(file)
  })
}

const triggerProfileUpload = () => profileInput.value?.click()
const triggerCoverUpload = () => coverInput.value?.click()

const handleProfileUpload = async (event) => {
  const file = event.target.files[0]
  if (!file) return
  if (file.size > 5 * 1024 * 1024) return showNotification('حجم الصورة يجب أن لا يتجاوز 5MB', 'error')
  if (!file.type.startsWith('image/')) return showNotification('الرجاء اختيار صورة صالحة', 'error')
  isLoading.value = true
  try { profileFile.value = file; profilePreview.value = await createPreview(file); showNotification('✅ تم تحميل الصورة بنجاح', 'success') }
  catch (error) { showNotification('❌ فشل تحميل الصورة', 'error') }
  finally { isLoading.value = false }
}

const handleCoverUpload = async (event) => {
  const file = event.target.files[0]
  if (!file) return
  if (file.size > 5 * 1024 * 1024) return showNotification('حجم الصورة يجب أن لا يتجاوز 5MB', 'error')
  if (!file.type.startsWith('image/')) return showNotification('الرجاء اختيار صورة صالحة', 'error')
  isLoading.value = true
  try { coverFile.value = file; coverPreview.value = await createPreview(file); showNotification('✅ تم تحميل الغلاف بنجاح', 'success') }
  catch (error) { showNotification('❌ فشل تحميل الغلاف', 'error') }
  finally { isLoading.value = false }
}

// ===== VALIDATION =====
const validateStep1 = () => {
  errors.value = {}
  if (!form.fullName?.trim()) errors.value.fullName = 'الاسم الكامل مطلوب'
  if (!form.email?.trim()) errors.value.email = 'البريد الإلكتروني مطلوب'
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) errors.value.email = 'بريد إلكتروني غير صحيح'
  if (!form.phone?.trim()) errors.value.phone = 'رقم الهاتف مطلوب'
  else if (!/^\d{8,}$/.test(form.phone.replace(/\s/g, ''))) errors.value.phone = '8 أرقام على الأقل'
  if (!form.address?.trim()) errors.value.address = 'العنوان مطلوب'
  if (!form.password) errors.value.password = 'كلمة المرور مطلوبة'
  else if (form.password.length < 6) errors.value.password = '6 أحرف على الأقل'
  if (!form.confirmPassword) errors.value.confirmPassword = 'تأكيد كلمة المرور مطلوب'
  else if (form.password !== form.confirmPassword) errors.value.confirmPassword = 'كلمة المرور غير متطابقة'
  if (Object.keys(errors.value).length === 0) currentStep.value = 2
  else showNotification('الرجاء تصحيح الأخطاء', 'error')
}

const validateStep2 = () => {
  errors.value = {}
  if (!form.shopName?.trim()) errors.value.shopName = 'اسم المتجر مطلوب'
  if (!form.specialty) errors.value.specialty = 'التخصص مطلوب'
  if (!form.description?.trim()) errors.value.description = 'وصف النشاط مطلوب'
  else if (form.description.length < 10) errors.value.description = '10 أحرف على الأقل'
  if (Object.keys(errors.value).length === 0) currentStep.value = 3
  else showNotification('الرجاء تصحيح الأخطاء', 'error')
}

// ===== HANDLERS CODE INPUTS =====
const handleCodeInput = (event, index) => {
  let v = event.target.value.replace(/\D/g, '')
  if (!v) { verificationDigits.value[index] = ''; return }
  v = v.slice(-1)
  verificationDigits.value[index] = v
  if (v && index < 5) {
    const inputs = event.target.parentElement.querySelectorAll('.code-input-verify')
    if (inputs && inputs[index + 1]) inputs[index + 1].focus()
  }
}

const handleCodeDelete = (event, index) => {
  if (!verificationDigits.value[index] && index > 0) {
    const inputs = event.target.parentElement.querySelectorAll('.code-input-verify')
    if (inputs && inputs[index - 1]) inputs[index - 1].focus()
  }
}

const handleCodePaste = (event) => {
  const paste = (event.clipboardData?.getData('text') || '').replace(/\D/g, '').slice(0, 6)
  if (!paste) return
  event.preventDefault()
  for (let i = 0; i < 6; i++) verificationDigits.value[i] = paste[i] || ''
}

// ===== VÉRIFICATION CODE =====
const getVendorVerifyCode = () => verificationDigits.value.join('')

const sendVendorVerificationCode = async () => {
  try {
    const emailToUse = savedEmailForVerification.value
    console.log('📧 Envoi code à:', emailToUse)
    const response = await api.post('/auth/send-email-verification-code', { email: emailToUse })
    console.log('📧 Réponse:', response.data)
    verificationDigits.value = ['', '', '', '', '', '']
    if (response.data.alreadyVerified) {
      showVerificationModal.value = false
      document.body.style.overflow = ''
      showSuccessModal.value = true
      document.body.style.overflow = 'hidden'
      showNotification('✅ البريد الإلكتروني مؤكد بالفعل', 'success')
    }
    return response.data
  } catch (error) {
    console.error('❌ Erreur envoi code:', error.response?.data || error.message)
    throw error
  }
}

const resendVendorCode = async () => {
  resendingCode.value = true
  try {
    await sendVendorVerificationCode()
    showNotification('📨 تم إرسال رمز جديد', 'success')
  } catch (error) {
    showNotification('❌ خطأ في إرسال الرمز', 'error')
  } finally {
    resendingCode.value = false
  }
}

const verifyVendorEmail = async () => {
  const code = getVendorVerifyCode()
  const emailToUse = savedEmailForVerification.value

  if (code.length !== 6) {
    showNotification('الرجاء إدخال الرمز كاملاً', 'warning')
    return
  }

  verifyingEmail.value = true
  try {
    const response = await api.post('/auth/verify-email-code', { email: emailToUse, code })
    if (response.data.success) {
      showVerificationModal.value = false
      document.body.style.overflow = ''
      showSuccessModal.value = true
      document.body.style.overflow = 'hidden'
      showNotification('✅ تم تأكيد البريد الإلكتروني', 'success')
    } else {
      showNotification(response.data.message || 'رمز غير صحيح', 'error')
    }
  } catch (error) {
    showNotification(error.response?.data?.message || 'خطأ في التحقق', 'error')
  } finally {
    verifyingEmail.value = false
  }
}

// ===== SUBMIT =====
const submitForm = async () => {
  if (!form.acceptTerms) return showNotification('يجب الموافقة على الشروط والأحكام', 'error')

  isSubmitting.value = true
  isLoading.value = true

  // ✅ Sauvegarder l'email AVANT tout
  savedEmailForVerification.value = form.email.toLowerCase().trim()

  try {
    // ✅ Si PAS de fichiers, envoyer en JSON
    if (!profileFile.value && !coverFile.value) {
      const response = await api.post('/auth/register-vendor', {
        fullName: form.fullName,
        email: savedEmailForVerification.value,
        phone: form.phone.replace(/\s/g, ''),
        address: form.address,
        password: form.password,
        shopName: form.shopName,
        specialty: form.specialty,
        description: form.description,
        location: 'تونس',
        experience: form.experience || 0
      })

      if (response.data.success) {
        if (response.data.token) {
          localStorage.setItem('token', response.data.token)
          api.defaults.headers.common['Authorization'] = `Bearer ${response.data.token}`
        }
        if (response.data.user) {
          localStorage.setItem('user', JSON.stringify(response.data.user))
          authStore.user = response.data.user
        }

        // Reset form
        form.fullName = ''; form.email = ''; form.phone = ''; form.address = ''
        form.password = ''; form.confirmPassword = ''
        form.shopName = ''; form.specialty = ''; form.experience = 0
        form.description = ''; form.acceptTerms = false
        profileFile.value = null; coverFile.value = null
        profilePreview.value = null; coverPreview.value = null
        verificationDigits.value = ['', '', '', '', '', '']
        currentStep.value = 1; errors.value = {}

        // Afficher modal vérification
        showVerificationModal.value = true
        document.body.style.overflow = 'hidden'

        await sendVendorVerificationCode()
        showNotification('📧 تم إرسال رمز التحقق إلى بريدك الإلكتروني', 'success')
      } else {
        throw new Error(response.data.message || 'Erreur inscription')
      }
      return
    }

    // ✅ Si fichiers, envoyer en FormData
    const formData = new FormData()
    formData.append('fullName', form.fullName)
    formData.append('email', savedEmailForVerification.value)
    formData.append('phone', form.phone.replace(/\s/g, ''))
    formData.append('address', form.address)
    formData.append('password', form.password)
    formData.append('shopName', form.shopName)
    formData.append('specialty', form.specialty)
    formData.append('description', form.description)
    formData.append('location', 'تونس')
    formData.append('experience', form.experience || 0)
    if (profileFile.value) formData.append('avatar', profileFile.value)
    if (coverFile.value) formData.append('coverImage', coverFile.value)

    const response = await api.post('/auth/register-vendor', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })

    if (response.data.success) {
      if (response.data.token) {
        localStorage.setItem('token', response.data.token)
        api.defaults.headers.common['Authorization'] = `Bearer ${response.data.token}`
      }
      if (response.data.user) {
        localStorage.setItem('user', JSON.stringify(response.data.user))
        authStore.user = response.data.user
      }

      // Reset form
      form.fullName = ''; form.email = ''; form.phone = ''; form.address = ''
      form.password = ''; form.confirmPassword = ''
      form.shopName = ''; form.specialty = ''; form.experience = 0
      form.description = ''; form.acceptTerms = false
      profileFile.value = null; coverFile.value = null
      profilePreview.value = null; coverPreview.value = null
      verificationDigits.value = ['', '', '', '', '', '']
      currentStep.value = 1; errors.value = {}

      // Afficher modal vérification
      showVerificationModal.value = true
      document.body.style.overflow = 'hidden'

      await sendVendorVerificationCode()
      showNotification('📧 تم إرسال رمز التحقق إلى بريدك الإلكتروني', 'success')
    } else {
      throw new Error(response.data.message || 'Erreur inscription')
    }
  } catch (error) {
    console.error('❌ Erreur submitForm:', error)
    let errorMessage = 'حدث خطأ غير متوقع'
    if (error.response?.data?.message) errorMessage = error.response.data.message
    else if (error.message?.includes('duplicate')) errorMessage = '⚠️ هذا البريد الإلكتروني مسجل بالفعل'
    showNotification(errorMessage, 'error')
    resetForm()
  } finally {
    isSubmitting.value = false
    isLoading.value = false
  }
}

// ===== MODALS =====
const closeSuccessModal = () => {
  showSuccessModal.value = false
  document.body.style.overflow = ''
  savedEmailForVerification.value = ''
  router.push('/')
}

// ===== LIFECYCLE =====
onMounted(() => {
  resetForm()
  if (authStore.isAuthenticated) {
    const userRole = authStore.user?.role
    if (userRole === 'vendor') router.push(`/vendor/${authStore.user?.vendorId}`)
    else if (userRole === 'pending') router.push('/pending-vendor')
    else router.push('/')
  }
})
</script>

<style scoped>
@import url('https://fonts.googleapis.com/css2?family=Amiri:ital,wght@0,400;0,700;1,400;1,700&display=swap');

* { margin: 0; padding: 0; box-sizing: border-box; }

.become-vendor-page {
  min-height: 100vh;
  background: linear-gradient(135deg, #08717f 0%, #d40025 100%);
  font-family: 'Amiri', 'Cairo', serif;
  direction: rtl;
  display: flex;
  align-items: center;
  justify-content: center;
  padding: 20px;
}

.become-vendor-page,
.become-vendor-page * {
  font-family: 'Amiri', 'Cairo', serif;
}

.vendor-wrapper {
  display: flex;
  max-width: 1400px;
  width: 100%;
  background: white;
  border-radius: 32px;
  overflow: hidden;
  box-shadow: 0 25px 50px rgba(0,0,0,0.2);
}

.brand-column {
  flex: 1;
  background: linear-gradient(135deg, #08717f 0%, #d40025 100%);
  padding: 48px 40px;
  display: flex;
  align-items: center;
  justify-content: center;
  position: relative;
  overflow: hidden;
}

.brand-column::before {
  content: '';
  position: absolute;
  inset: 0;
  background: url('/src/assets/Gemini_Generated_Image_il5b9til5b9til5b.png');
  background-size: cover;
  opacity: 0.15;
  animation: slowZoom 20s infinite alternate;
}

.brand-content {
  position: relative;
  z-index: 2;
  text-align: center;
}

.brand-logo { width: 120px; margin-bottom: 32px; filter: brightness(0) invert(1); animation: floatLogo 3s infinite; }

.welcome-title { font-size: 2rem; font-weight: 800; color: white; margin-bottom: 12px; text-shadow: 0 2px 10px rgba(0,0,0,0.2); }
.welcome-text { font-size: 1.1rem; color: white; opacity: 0.95; margin-bottom: 8px; }
.welcome-subtext { font-size: 0.9rem; color: white; opacity: 0.85; }

.form-column { flex: 1; padding: 48px 40px; background: white; overflow-y: auto; max-height: 90vh; }

.progress-container { margin-bottom: 40px; padding: 10px; background: #f1f5f9; border-radius: 60px; }
.progress-steps { display: flex; align-items: center; justify-content: space-between; }
.step-wrapper { display: flex; align-items: center; flex: 1; }
.step { display: flex; flex-direction: column; align-items: center; }
.step-number {
  width: 44px; height: 44px; background: white; border: 2px solid #e2e8f0;
  border-radius: 50%; display: flex; align-items: center; justify-content: center;
  font-weight: 800; font-size: 1.1rem; color: #64748b; margin-bottom: 8px;
  transition: all 0.3s;
}
.step.active .step-number { background: linear-gradient(135deg, #08717f, #d40025); border-color: transparent; color: white; }
.step.completed .step-number { background: #10b981; border-color: transparent; color: white; }
.step-label { font-size: 0.7rem; color: #64748b; font-weight: 600; white-space: nowrap; }
.step.active .step-label { color: #08717f; font-weight: 700; }
.step-line { flex: 1; height: 2px; background: #e2e8f0; margin: 0 5px; margin-bottom: 25px; border-radius: 2px; }
.step-line.active { background: linear-gradient(135deg, #08717f, #d40025); }

.step-header { margin-bottom: 30px; }
.step-badge { display: inline-flex; align-items: center; gap: 8px; padding: 6px 16px; background: #f1f5f9; border-radius: 30px; font-size: 0.8rem; font-weight: 600; color: #08717f; margin-bottom: 15px; }
.step-title { font-size: 1.8rem; font-weight: 800; color: #1e293b; margin-bottom: 8px; }
.step-description { color: #64748b; font-size: 0.9rem; }

.auth-form { display: flex; flex-direction: column; gap: 20px; }
.form-group { display: flex; flex-direction: column; gap: 8px; }
.form-label { display: flex; align-items: center; gap: 8px; font-size: 0.9rem; font-weight: 700; color: #1e293b; margin-bottom: 10px; }
.form-label span:first-child { font-size: 1.2rem; }
.required { color: #d40025; font-size: 0.75rem; }
.optional { color: #64748b; font-size: 0.75rem; font-weight: normal; }

.form-input, .form-select, .form-textarea {
  width: 100%; padding: 14px 18px; border: 2px solid #e2e8f0; border-radius: 16px;
  font-size: 0.95rem; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); background: white; color: #1e293b;
}
.form-input:focus, .form-select:focus, .form-textarea:focus {
  outline: none; border-color: #08717f; box-shadow: 0 0 0 4px rgba(8,113,127,0.1);
}

/* Experience Input */
.experience-input {
  text-align: center !important;
  font-size: 1.2rem !important;
  font-weight: 700 !important;
  color: #08717f !important;
  background: #f8fafc !important;
  height: 52px;
  -moz-appearance: textfield;
}
.experience-input::-webkit-outer-spin-button,
.experience-input::-webkit-inner-spin-button {
  -webkit-appearance: none;
  margin: 0;
}
.experience-input:focus {
  border-color: #08717f !important;
  background: white !important;
  box-shadow: 0 0 0 4px rgba(8, 113, 127, 0.1) !important;
  transform: translateY(-1px);
}

/* Textarea */
.form-textarea {
  min-height: 120px;
  resize: vertical;
  line-height: 1.8;
  font-family: 'Amiri', 'Cairo', serif;
}
.form-textarea::placeholder {
  color: #94a3b8;
  font-size: 0.85rem;
  animation: placeholderPulse 2s ease-in-out infinite;
}

.phone-input-wrapper { display: flex; align-items: center; border: 2px solid #e2e8f0; border-radius: 16px; overflow: hidden; }
.phone-input-wrapper:focus-within { border-color: #08717f; box-shadow: 0 0 0 4px rgba(8,113,127,0.1); }
.country-code { padding: 14px 18px; background: #f1f5f9; color: #08717f; font-weight: 700; border-left: 2px solid #e2e8f0; }
.phone-field { flex: 1; border: none !important; }

.password-wrapper { position: relative; }
.toggle-password { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; font-size: 1.1rem; }

.form-select { appearance: none; padding-left: 40px; cursor: pointer; }

.char-counter {
  text-align: left; font-size: 0.8rem; color: #64748b; margin-top: 8px;
  font-weight: 500; transition: color 0.3s ease;
}
.char-counter.warning { color: #f59e0b; }
.char-counter.danger { color: #ef4444; }

.form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; align-items: start; }

.has-error .form-input, .has-error .form-select, .has-error .form-textarea,
.has-error .phone-input-wrapper { border-color: #ef4444 !important; }
.has-error .form-textarea,
.has-error input[type="number"].form-input {
  background: #fef2f2 !important;
}
.error-message { color: #ef4444; font-size: 0.75rem; margin-top: 4px; }

.upload-section { margin-bottom: 20px; }
.upload-area {
  border: 2px dashed #e2e8f0; border-radius: 24px; padding: 30px; text-align: center;
  cursor: pointer; transition: all 0.3s; background: #f8fafc; min-height: 160px;
  display: flex; align-items: center; justify-content: center;
}
.upload-area:hover { border-color: #08717f; background: #f1f5f9; transform: translateY(-2px); }
.upload-area.has-preview { border-style: solid; border-color: #08717f; }
.upload-icon { font-size: 2.5rem; margin-bottom: 12px; animation: float 3s infinite; }
.upload-text { color: #1e293b; font-weight: 600; margin-bottom: 5px; }
.upload-hint { color: #64748b; font-size: 0.75rem; }
.upload-preview { width: 100%; display: flex; align-items: center; justify-content: center; position: relative; }
.upload-preview img { max-width: 100%; max-height: 150px; border-radius: 16px; }
.cover-preview { width: 100%; height: 120px; background-size: cover; background-position: center; border-radius: 16px; }
.change-photo-btn {
  position: absolute; bottom: 10px; right: 10px; padding: 6px 12px;
  background: rgba(255,255,255,0.9); border: none; border-radius: 30px;
  cursor: pointer; display: flex; align-items: center; gap: 5px;
  font-weight: 600; font-size: 0.75rem;
}
.change-photo-btn:hover { background: #08717f; color: white; }

.summary-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0; }
.summary-card { background: #f8fafc; border-radius: 16px; padding: 20px; border: 1px solid #e2e8f0; transition: all 0.3s; }
.summary-card:hover { border-color: #08717f; transform: translateY(-2px); }
.summary-header { display: flex; align-items: center; gap: 10px; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 2px solid #e2e8f0; }
.summary-header h3 { font-size: 0.9rem; color: #1e293b; font-weight: 700; }
.summary-content { display: flex; flex-direction: column; gap: 10px; }
.summary-item { display: flex; justify-content: space-between; font-size: 0.8rem; }
.summary-item span:first-child { color: #64748b; font-weight: 600; }
.summary-item span:last-child { color: #1e293b; font-weight: 600; }

.terms-section { margin: 15px 0; padding: 15px; background: #f8fafc; border-radius: 16px; border: 1px solid #e2e8f0; }
.checkbox-label { display: flex; align-items: center; gap: 10px; cursor: pointer; position: relative; }
.checkbox-label input { position: absolute; opacity: 0; }
.checkmark { width: 20px; height: 20px; border: 2px solid #e2e8f0; border-radius: 5px; background: white; transition: all 0.3s; flex-shrink: 0; }
.checkbox-label input:checked ~ .checkmark { background: linear-gradient(135deg, #08717f, #d40025); border-color: transparent; }
.checkbox-label input:checked ~ .checkmark::after { content: '✓'; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: white; font-size: 0.7rem; font-weight: bold; }
.checkbox-text { color: #334155; font-size: 0.85rem; }
.terms-link { color: #08717f; text-decoration: none; font-weight: 600; }
.terms-link:hover { color: #d40025; }

.form-actions { display: flex; gap: 15px; margin-top: 20px; }
.btn-prev, .btn-next, .btn-submit {
  flex: 1; padding: 14px 32px; border: none; border-radius: 50px;
  font-size: 1rem; font-weight: 700; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
  display: flex; align-items: center; justify-content: center; gap: 10px;
  position: relative; overflow: hidden; font-family: 'Amiri', 'Cairo', serif;
}
.btn-prev { background: #f1f5f9; color: #475569; border: 1px solid #e2e8f0; }
.btn-prev:hover { background: #e2e8f0; transform: translateX(5px); color: #1e293b; }
.btn-next {
  background: linear-gradient(135deg, #08717f 0%, #0a94a6 100%);
  color: white;
  box-shadow: 0 4px 15px rgba(8, 113, 127, 0.3);
}
.btn-next:hover { transform: translateX(-5px); box-shadow: 0 8px 25px rgba(8, 113, 127, 0.4); }
.btn-next::before {
  content: ''; position: absolute; top: 0; left: -100%; width: 100%; height: 100%;
  background: linear-gradient(90deg, transparent, rgba(255,255,255,0.3), transparent);
  transition: left 0.5s ease;
}
.btn-next:hover::before { left: 100%; }
.btn-next:active { transform: translateX(-2px) scale(0.98); }

.btn-submit {
  background: linear-gradient(135deg, #d40025 0%, #ff1744 100%);
  color: white;
  box-shadow: 0 4px 15px rgba(212, 0, 37, 0.3);
}
.btn-submit:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(212, 0, 37, 0.4); }
.btn-submit:disabled { opacity: 0.6; cursor: not-allowed; }
.btn-submit::before {
  content: ''; position: absolute; top: 0; left: -100%; width: 100%; height: 100%;
  background: linear-gradient(90deg, transparent, rgba(255,255,255,0.3), transparent);
  transition: left 0.5s ease;
}
.btn-submit:hover::before { left: 100%; }

.login-link { margin-top: 30px; text-align: center; padding-top: 20px; border-top: 1px solid #e2e8f0; }
.login-link p { color: #64748b; margin-bottom: 10px; }
.login-link-btn { display: inline-flex; align-items: center; gap: 8px; color: #08717f; text-decoration: none; font-weight: 600; transition: all 0.3s; }
.login-link-btn:hover { color: #d40025; transform: translateX(-5px); }

.toast-notification { position: fixed; bottom: 30px; right: 30px; background: white; border-radius: 50px; box-shadow: 0 25px 50px rgba(0,0,0,0.2); z-index: 9999; min-width: 300px; overflow: hidden; }
.toast-content { display: flex; align-items: center; gap: 12px; padding: 16px 24px; }
.toast-progress { position: absolute; bottom: 0; left: 0; height: 3px; background: linear-gradient(90deg, #10b981, #08717f); animation: progress 3s linear; }
.toast-notification.success { border-right: 4px solid #10b981; }
.toast-notification.error { border-right: 4px solid #ef4444; }

.loading-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.7); backdrop-filter: blur(5px); display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 10000; }
.spinner { width: 50px; height: 50px; border: 4px solid #e2e8f0; border-top: 4px solid #08717f; border-right: 4px solid #d40025; border-radius: 50%; animation: spin 1s linear infinite; margin-bottom: 20px; }
.loader { width: 20px; height: 20px; border: 2px solid rgba(255,255,255,0.3); border-top: 2px solid white; border-radius: 50%; animation: spin 0.8s linear infinite; }

@keyframes slowZoom { 0% { transform: scale(1); } 100% { transform: scale(1.1); } }
@keyframes floatLogo { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-5px); } }
@keyframes float { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-10px); } }
@keyframes spin { to { transform: rotate(360deg); } }
@keyframes progress { from { width: 0; } to { width: 100%; } }
@keyframes placeholderPulse { 0%,100% { opacity: 0.6; } 50% { opacity: 1; } }

.dark-mode { /* Inherited from parent */ }
.become-vendor-page.dark-mode { background: linear-gradient(135deg, #065a69 0%, #b00020 100%); }
.dark-mode .vendor-wrapper { background: #1f2937; }
.dark-mode .brand-column { background: linear-gradient(135deg, #065a69 0%, #b00020 100%); }
.dark-mode .form-column { background: #1f2937; }
.dark-mode .progress-container { background: #374151; }
.dark-mode .step-number { background: #1f2937; border-color: #4b5563; color: #9ca3af; }
.dark-mode .step.active .step-number { background: linear-gradient(135deg, #3b82f6, #2563eb); }
.dark-mode .step.completed .step-number { background: #10b981; }
.dark-mode .step-label, .dark-mode .step-description, .dark-mode .char-counter { color: #9ca3af; }
.dark-mode .step-title, .dark-mode .summary-header h3, .dark-mode .summary-item span:last-child { color: #f3f4f6; }
.dark-mode .form-label, .dark-mode .upload-text { color: #e5e7eb; }
.dark-mode .form-input, .dark-mode .form-select, .dark-mode .form-textarea { background: #374151; border-color: #4b5563; color: #f3f4f6; }
.dark-mode .experience-input { background: #374151 !important; color: #60a5fa !important; border-color: #4b5563 !important; }
.dark-mode .experience-input:focus { background: #1f2937 !important; border-color: #60a5fa !important; box-shadow: 0 0 0 4px rgba(96, 165, 250, 0.2) !important; }
.dark-mode .form-textarea { background: #374151; border-color: #4b5563; color: #f3f4f6; }
.dark-mode .form-textarea:focus { border-color: #60a5fa; background: #1f2937; box-shadow: 0 0 0 4px rgba(96, 165, 250, 0.2); }
.dark-mode .form-textarea::placeholder { color: rgba(255,255,255,0.3); }
.dark-mode .phone-input-wrapper, .dark-mode .upload-area { background: #374151; border-color: #4b5563; }
.dark-mode .country-code { background: #374151; border-left-color: #4b5563; color: #3b82f6; }
.dark-mode .summary-card, .dark-mode .terms-section { background: #374151; border-color: #4b5563; }
.dark-mode .btn-prev { background: #374151; color: #9ca3af; border-color: #4b5563; }
.dark-mode .btn-prev:hover { background: #4b5563; color: #f3f4f6; }
.dark-mode .btn-next { background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); }
.dark-mode .btn-submit { background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); }
.dark-mode .toast-notification { background: #1f2937; }
.dark-mode .toast-message { color: #f3f4f6; }
.dark-mode .has-error .form-textarea,
.dark-mode .has-error input[type="number"].form-input { background: rgba(239, 68, 68, 0.1) !important; }
.dark-mode .char-counter.warning { color: #fbbf24; }
.dark-mode .char-counter.danger { color: #fca5a5; }

/* Success Popup Styles */
.success-popup-overlay {
  position: fixed; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(8px);
  display: flex; align-items: center; justify-content: center; z-index: 10000; padding: 20px;
  animation: fadeIn 0.3s ease;
}
.success-popup {
  position: relative; background: #ffffff; border-radius: 24px; padding: 36px 28px 28px;
  max-width: 480px; width: 100%; text-align: center;
  box-shadow: 0 25px 60px rgba(0,0,0,0.15);
  animation: slideUp 0.4s cubic-bezier(0.22, 1, 0.36, 1);
  direction: rtl; font-family: 'Amiri', 'Cairo', serif; max-height: 90vh; overflow-y: auto;
}
.success-popup.dark-mode { background: #1f2937; border: 1px solid rgba(255,255,255,0.08); box-shadow: 0 25px 60px rgba(0,0,0,0.5); }
.popup-close-btn {
  position: absolute; top: 14px; left: 14px; width: 36px; height: 36px;
  background: #f1f5f9; border: 1px solid #e2e8f0; border-radius: 50%; cursor: pointer;
  display: flex; align-items: center; justify-content: center; color: #64748b; transition: all 0.3s ease;
}
.popup-close-btn svg { width: 18px; height: 18px; }
.popup-close-btn:hover { background: #ef4444; color: white; border-color: #ef4444; transform: rotate(90deg); }
.dark-mode .popup-close-btn { background: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.12); color: rgba(255,255,255,0.5); }
.dark-mode .popup-close-btn:hover { background: rgba(239,68,68,0.25); color: #fca5a5; }
.popup-logo-container { margin-bottom: 16px; }
.popup-logo-img { height: 40px; }
.dark-mode .popup-logo-img { filter: brightness(0) invert(1); }
.popup-icon-container { margin-bottom: 16px; display: flex; justify-content: center; }
.icon-circle-success {
  width: 60px; height: 60px; border-radius: 50%;
  background: linear-gradient(135deg, #10b981, #059669);
  display: flex; align-items: center; justify-content: center;
  box-shadow: 0 0 30px rgba(16,185,129,0.3);
}
.icon-circle-success svg { width: 30px; height: 30px; stroke: white; }
.dark-mode .icon-circle-success { box-shadow: 0 0 30px rgba(16,185,129,0.4); }
.popup-title { font-size: 1.4rem; font-weight: 800; color: #1e293b; margin-bottom: 4px; }
.dark-mode .popup-title { color: #ffffff; }
.popup-subtitle { font-size: 0.9rem; color: #64748b; margin-bottom: 20px; }
.dark-mode .popup-subtitle { color: rgba(255,255,255,0.45); }
.popup-message-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 14px; padding: 16px; margin-bottom: 16px; text-align: right; }
.dark-mode .popup-message-box { background: rgba(255,255,255,0.04); border-color: rgba(255,255,255,0.06); }
.message-emoji { font-size: 1.6rem; display: block; margin-bottom: 10px; text-align: center; }
.popup-message-box p { font-size: 0.9rem; color: #475569; line-height: 1.8; margin: 0; }
.dark-mode .popup-message-box p { color: rgba(255,255,255,0.7); }
.text-gold { color: #d97706; font-weight: 700; }
.dark-mode .text-gold { color: #fbbf24; }
.popup-email-box {
  display: flex; align-items: center; gap: 14px; background: #f0fdf4;
  border: 1px solid #bbf7d0; border-radius: 16px; padding: 16px; margin-bottom: 16px; text-align: right;
}
.dark-mode .popup-email-box { background: rgba(8,113,127,0.1); border-color: rgba(8,113,127,0.2); }
.email-image { width: 50px; height: 50px; border-radius: 12px; object-fit: cover; flex-shrink: 0; }
.email-content { flex: 1; }
.email-icon-row { display: flex; align-items: center; gap: 6px; margin-bottom: 4px; }
.email-emoji { font-size: 1rem; }
.email-text { font-size: 0.8rem; color: #475569; margin: 0; }
.dark-mode .email-text { color: rgba(255,255,255,0.6); }
.email-highlight { display: block; font-size: 0.95rem; font-weight: 700; color: #08717f; }
.dark-mode .email-highlight { color: #2dd4bf; }
.email-subtext { font-size: 0.75rem; color: #64748b; margin: 0; }
.dark-mode .email-subtext { color: rgba(255,255,255,0.4); }
.popup-contact-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 14px; padding: 14px; margin-bottom: 20px; }
.dark-mode .popup-contact-box { background: rgba(255,255,255,0.03); border-color: rgba(255,255,255,0.06); }
.contact-row { display: flex; align-items: center; justify-content: center; gap: 6px; color: #475569; margin-bottom: 8px; font-size: 0.9rem; }
.dark-mode .contact-row { color: rgba(255,255,255,0.55); }
.contact-link { display: block; font-weight: 700; color: #08717f; text-decoration: none; direction: ltr; margin-bottom: 8px; }
.dark-mode .contact-link { color: #60a5fa; }
.contact-link:hover { text-decoration: underline; }
.contact-hint { font-size: 0.75rem; color: #64748b; margin: 0; }
.dark-mode .contact-hint { color: rgba(255,255,255,0.35); }
.popup-home-btn {
  width: 100%; padding: 14px 20px; background: linear-gradient(135deg, #08717f, #065a69);
  color: white; border: none; border-radius: 14px; font-size: 1rem; font-weight: 700;
  cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px;
  transition: all 0.3s ease; font-family: 'Amiri', 'Cairo', serif;
}
.popup-home-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(8,113,127,0.35); }
.dark-mode .popup-home-btn { background: linear-gradient(135deg, #0a94a6, #08717f); }

.code-input-verify {
  width: 48px; height: 56px; border: 2px solid #e2e8f0; border-radius: 14px;
  text-align: center; font-size: 1.4rem; font-weight: 700; background: white; transition: all 0.3s ease;
}
.code-input-verify:focus { outline: none; border-color: #08717f; box-shadow: 0 0 0 3px rgba(8,113,127,0.2); }
.dark-mode .code-input-verify { background: #374151; border-color: #4b5563; color: #f3f4f6; }

@keyframes slideUp { from { opacity: 0; transform: translateY(30px) scale(0.95); } to { opacity: 1; transform: translateY(0) scale(1); } }
@keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }

/* Responsive */
@media (max-width: 1024px) {
  .vendor-wrapper { flex-direction: column; max-width: 600px; }
  .brand-column, .form-column { padding: 40px 30px; }
}
@media (max-width: 768px) {
  .become-vendor-page { padding: 16px; }
  .step-label { font-size: 0.6rem; }
  .step-number { width: 36px; height: 36px; font-size: 0.9rem; }
  .step-title { font-size: 1.4rem; }
  .form-row, .summary-grid { grid-template-columns: 1fr; }
  .form-actions { flex-direction: column; }
  .btn-prev, .btn-next, .btn-submit { width: 100%; justify-content: center; }
  .btn-next:hover { transform: translateY(-2px); }
  .btn-prev:hover { transform: translateY(-2px); }
  .toast-notification { width: calc(100% - 32px); right: 16px; bottom: 16px; }
}
@media (max-width: 480px) {
  .welcome-title { font-size: 1.5rem; }
  .step-title { font-size: 1.2rem; }
  .phone-input-wrapper { flex-direction: column; }
  .country-code { border-left: none; border-bottom: 2px solid #e2e8f0; text-align: center; }
  .summary-item { flex-direction: column; align-items: flex-start; gap: 5px; }
  .experience-input { font-size: 1rem !important; padding: 12px 16px; }
  .form-textarea { font-size: 0.9rem; padding: 12px 16px; min-height: 80px; }
  .form-label { font-size: 0.8rem; }
}
/* ===== DARK MODE COMPLET POUR BECOME VENDOR ===== */
/* Ajoutez ces styles à la fin de votre section <style scoped> */

/* Fond général */
.become-vendor-page.dark-mode {
  background: linear-gradient(135deg, #065a69 0%, #b00020 100%) !important;
}

/* Wrapper */
.become-vendor-page.dark-mode .vendor-wrapper {
  background: #1e1e30 !important;
}

/* Colonne gauche - Branding (garde le dégradé) */
.become-vendor-page.dark-mode .brand-column {
  background: linear-gradient(135deg, #065a69 0%, #b00020 100%) !important;
}

/* Colonne droite - Formulaire */
.become-vendor-page.dark-mode .form-column {
  background: #1e1e30 !important;
}

/* Progress Container */
.become-vendor-page.dark-mode .progress-container {
  background: #121220 !important;
}

/* Progress Steps */
.become-vendor-page.dark-mode .step-number {
  background: #1e1e30 !important;
  border-color: #2a2a40 !important;
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .step.active .step-number {
  background: linear-gradient(135deg, #2dd4bf, #0a94a6) !important;
  border-color: transparent !important;
  color: white !important;
}

.become-vendor-page.dark-mode .step.completed .step-number {
  background: #10b981 !important;
  border-color: transparent !important;
  color: white !important;
}

.become-vendor-page.dark-mode .step-label {
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .step.active .step-label {
  color: #2dd4bf !important;
}

.become-vendor-page.dark-mode .step-line {
  background: #2a2a40 !important;
}

.become-vendor-page.dark-mode .step-line.active {
  background: linear-gradient(135deg, #2dd4bf, #0a94a6) !important;
}

/* Step Header */
.become-vendor-page.dark-mode .step-badge {
  background: #121220 !important;
  color: #2dd4bf !important;
}

.become-vendor-page.dark-mode .step-title {
  color: #f1f5f9 !important;
}

.become-vendor-page.dark-mode .step-description {
  color: #94a3b8 !important;
}

/* Labels */
.become-vendor-page.dark-mode .form-label {
  color: #cbd5e1 !important;
}

/* Inputs */
.become-vendor-page.dark-mode .form-input,
.become-vendor-page.dark-mode .form-select,
.become-vendor-page.dark-mode .form-textarea {
  background: #121220 !important;
  border-color: #2a2a40 !important;
  color: #f1f5f9 !important;
}

.become-vendor-page.dark-mode .form-input:focus,
.become-vendor-page.dark-mode .form-select:focus,
.become-vendor-page.dark-mode .form-textarea:focus {
  border-color: #2dd4bf !important;
  box-shadow: 0 0 0 4px rgba(45, 212, 191, 0.15) !important;
}

/* Experience Input */
.become-vendor-page.dark-mode .experience-input {
  background: #121220 !important;
  color: #2dd4bf !important;
  border-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .experience-input:focus {
  background: #1a1a2e !important;
  border-color: #2dd4bf !important;
}

/* Phone Input */
.become-vendor-page.dark-mode .phone-input-wrapper {
  background: #121220 !important;
  border-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .country-code {
  background: #1e1e30 !important;
  color: #2dd4bf !important;
  border-left-color: #2a2a40 !important;
}

/* Password Toggle */
.become-vendor-page.dark-mode .toggle-password {
  color: #94a3b8 !important;
}

/* Char Counter */
.become-vendor-page.dark-mode .char-counter {
  color: #94a3b8 !important;
}

/* Upload Area */
.become-vendor-page.dark-mode .upload-area {
  background: #121220 !important;
  border-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .upload-area:hover {
  background: #1a1a2e !important;
  border-color: #2dd4bf !important;
}

.become-vendor-page.dark-mode .upload-text {
  color: #f1f5f9 !important;
}

.become-vendor-page.dark-mode .upload-hint {
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .change-photo-btn {
  background: rgba(30, 30, 48, 0.9) !important;
  color: #f1f5f9 !important;
}

/* Summary Cards */
.become-vendor-page.dark-mode .summary-card {
  background: #121220 !important;
  border-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .summary-header {
  border-bottom-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .summary-header h3 {
  color: #f1f5f9 !important;
}

.become-vendor-page.dark-mode .summary-item span:first-child {
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .summary-item span:last-child {
  color: #f1f5f9 !important;
}

/* Terms Section */
.become-vendor-page.dark-mode .terms-section {
  background: #121220 !important;
  border-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .checkbox-text {
  color: #cbd5e1 !important;
}

.become-vendor-page.dark-mode .terms-link {
  color: #2dd4bf !important;
}

.become-vendor-page.dark-mode .checkmark {
  background: #1e1e30 !important;
  border-color: #2a2a40 !important;
}

/* Buttons */
.become-vendor-page.dark-mode .btn-prev {
  background: #2a2a40 !important;
  color: #94a3b8 !important;
  border-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .btn-prev:hover {
  background: #3a3a55 !important;
  color: #f1f5f9 !important;
}

.become-vendor-page.dark-mode .btn-next {
  background: linear-gradient(135deg, #08717f, #065a69) !important;
}

.become-vendor-page.dark-mode .btn-submit {
  background: linear-gradient(135deg, #d40025, #b00020) !important;
}

/* Login Link */
.become-vendor-page.dark-mode .login-link {
  border-top-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .login-link p {
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .login-link-btn {
  color: #2dd4bf !important;
}

/* Error States */
.become-vendor-page.dark-mode .has-error .form-input,
.become-vendor-page.dark-mode .has-error .form-select,
.become-vendor-page.dark-mode .has-error .form-textarea {
  border-color: #ef4444 !important;
}

.become-vendor-page.dark-mode .has-error .form-textarea,
.become-vendor-page.dark-mode .has-error input[type="number"].form-input {
  background: rgba(239, 68, 68, 0.1) !important;
}

/* Loading Overlay */
.become-vendor-page.dark-mode .loading-overlay {
  background: rgba(0, 0, 0, 0.85) !important;
}

.become-vendor-page.dark-mode .loading-overlay p {
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .spinner {
  border-color: #2a2a40 !important;
  border-top-color: #2dd4bf !important;
  border-right-color: #ef4444 !important;
}

/* Toast */
.become-vendor-page.dark-mode .toast-notification {
  background: #1e1e30 !important;
  box-shadow: 0 25px 50px rgba(0, 0, 0, 0.3) !important;
}

.become-vendor-page.dark-mode .toast-message {
  color: #f1f5f9 !important;
}

/* Success Popup */
.become-vendor-page.dark-mode .success-popup {
  background: #1e1e30 !important;
  border-color: #2a2a40 !important;
  box-shadow: 0 25px 60px rgba(0, 0, 0, 0.5) !important;
}

.become-vendor-page.dark-mode .popup-title {
  color: #f1f5f9 !important;
}

.become-vendor-page.dark-mode .popup-subtitle {
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .popup-message-box {
  background: #121220 !important;
  border-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .popup-message-box p {
  color: #cbd5e1 !important;
}

.become-vendor-page.dark-mode .popup-email-box {
  background: #0a1f1a !important;
  border-color: #14532d !important;
}

.become-vendor-page.dark-mode .email-text {
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .email-highlight {
  color: #2dd4bf !important;
}

.become-vendor-page.dark-mode .email-subtext {
  color: #64748b !important;
}

.become-vendor-page.dark-mode .popup-contact-box {
  background: #121220 !important;
  border-color: #2a2a40 !important;
}

.become-vendor-page.dark-mode .contact-row {
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .contact-link {
  color: #60a5fa !important;
}

.become-vendor-page.dark-mode .contact-hint {
  color: #64748b !important;
}

.become-vendor-page.dark-mode .popup-close-btn {
  background: #2a2a40 !important;
  border-color: #2a2a40 !important;
  color: #94a3b8 !important;
}

.become-vendor-page.dark-mode .popup-logo-img {
  filter: brightness(0) invert(1) !important;
}

/* Code Inputs */
.become-vendor-page.dark-mode .code-input-verify {
  background: #121220 !important;
  border-color: #2a2a40 !important;
  color: #f1f5f9 !important;
}

.become-vendor-page.dark-mode .code-input-verify:focus {
  border-color: #2dd4bf !important;
}

/* Form Textarea Placeholder */
.become-vendor-page.dark-mode .form-textarea::placeholder {
  color: #64748b !important;
}

/* Dark mode pour le bouton resend */
.become-vendor-page.dark-mode .success-popup button[style*="background: #f1f5f9"] {
  background: #2a2a40 !important;
  color: #f1f5f9 !important;
}
</style>
