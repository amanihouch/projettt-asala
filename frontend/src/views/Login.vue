<!-- frontend/src/views/Login.vue - Version COMPLÈTE avec toutes les options -->
<template>
  <div class="login-page" dir="rtl">
    <!-- HONEYPOT FIELDS - Pour tromper l'autofill du navigateur -->
        ...
        <div class="form-card">
          ...
          <div v-if="currentView === 'login'" class="social-login">
            ...
            <button class="social-btn facebook" @click="handleFacebookLogin" :disabled="socialLoading === 'facebook'">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M24 12.07C24 5.41 18.63 0 12 0S0 5.41 0 12.07C0 18.1 4.39 23.06 10.13 24v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.68.24 2.68.24v2.96h-1.51c-1.49 0-1.95.93-1.95 1.88v2.26h3.32l-.53 3.49h-2.79V24C19.61 23.06 24 18.1 24 12.07z" fill="#1877F2"/>
              </svg>
              <span>{{ socialLoading === 'facebook' ? 'جاري التحميل...' : 'Facebook' }}</span>
            </button>
          </div>

          <div v-if="currentView === 'login'" class="divider">
          ...
          <div v-else class="form-container">
            <div class="register-progress">
              <div class="progress-steps">
                ...
                <div class="step" :class="{ active: registerStep >= 2 }">
                  ...
                  <span class="step-label">الصورة</span>
                </div>
              </div>
            </div>
            ...
            <div v-if="registerStep === 1">
              <form @submit.prevent="goToStep2" class="auth-form" autocomplete="off" novalidate>
                ...
                <div class="form-group" :class="{ 'has-error': errors.confirmPassword }">
                  ...
                  <input
                    :name="`confirm_password_${Date.now()}`"
                    type="password"
                    class="form-input"
                    placeholder="••••••••"
                    required
            ...
            <!-- Step 2 - Photo de profil avec image par défaut -->
                ...
                <div class="photo-upload-section">
                  <div class="avatar-upload-container">
                    <div class="avatar-preview-wrapper">
                      <img
                        :src="avatarPreview || defaultAvatarPath"
                        alt="Profile Avatar"
          ...
          <div class="action-buttons">
            <button class="action-btn artisan" @click="goToVendorRegister">
              <span>هل أنت حرفي؟</span>
              <span class="btn-highlight">انضم كبائع</span>
          ...
          <div v-else-if="forgotStep === 2" class="modal-body">
            ...
            <div class="modal-actions" style="flex-direction: column; gap: 10px;">
              <div style="display: flex; gap: 10px;">
                <button class="modal-btn primary" @click="verifyCode" :disabled="modalLoading" style="flex: 2;">
                  <span v-if="!modalLoading">تحقق</span>
                  <span v-else class="loading-spinner">
                    <span class="spinner"></span>
                  </span>
                </button>
                <button class="modal-btn secondary" @click="forgotStep = 1" style="flex: 1;">رجوع</button>
              </div>
              <button class="modal-btn secondary" @click="sendResetCode" :disabled="modalLoading">
                <span v-if="!modalLoading">إعادة إرسال الرمز</span>
                <span v-else class="loading-spinner">
                  <span class="spinner"></span>
                </span>
              </button>
...
// ===== CONSTANTES - IMAGES PAR DÉFAUT =====
const defaultAvatarPath = '/src/assets/default-profil.jpg'
const defaultCoverPath = '/src/assets/default-cover.jpg'
...
const strengthText = computed(() => {
  if (passwordStrength.value < 30) return 'ضعيفة'
  if (passwordStrength.value < 60) return 'متوسطة'
  return 'قوية'
})
...
const validateEmailFormat = () => {
  ...
  emailFormatValid.value = emailRegex.test(email)

  if (email && emailFormatValid.value) {
...
const checkEmailAvailability = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      emailValid.value = !response.data.exists
      if (response.data.exists) {
        errors.value.email = 'هذا البريد الإلكتروني مستخدم بالفعل'
      } else {
    ...
    console.warn('⚠️ Email check error, skipping:', error.message)
    emailValid.value = true
    delete errors.value.email
  } finally {
...
const clearFieldOnFocus = (event) => {
  if (event.target.value) {
    event.target.value = ''
    if (event.target.name?.startsWith('email') || event.target.name?.startsWith('password')) {
      if (event.target.name?.startsWith('email')) loginForm.email = ''
      if (event.target.name?.startsWith('password')) loginForm.password = ''
    }
  }
}
  ...
  try {
    ...
    if (result.success) {
      ...
      setTimeout(async () => {
        ...
        const role = user?.role || authStore.userRole

        if (role === 'admin') {
          router.push('/admin')
        } else if (role === 'vendor') {
          // ✅ CORRECTION : Récupérer le vrai vendorId
          const id = authStore.vendorId || await authStore.fetchVendorId()
          if (id) {
...
const handleGoogleLogin = () => {
  ...
  try {
    showNotification('جاري التوجه إلى Google...', 'info')
    // URL CORRECTE avec /api/v1/auth/google
    window.location.href = 'http://localhost:5000/api/v1/auth/google'
  } catch (error) {
    console.error('❌ Google login error:', error)
    showNotification('حدث خطأ أثناء تسجيل الدخول بواسطة Google', 'error')
...
const handleFacebookLogin = () => {
  ...
  try {
    showNotification('جاري التوجه إلى Facebook...', 'info')
    // URL CORRECTE avec /api/v1/auth/facebook
    window.location.href = 'http://localhost:5000/api/v1/auth/facebook'
  } catch (error) {
    console.error('❌ Facebook login error:', error)
    showNotification('حدث خطأ أثناء تسجيل الدخول بواسطة Facebook', 'error')
    socialLoading.value = null
  }
}
...
const validateStep1 = () => {
  ...
  if (!phoneRegex.test(registerForm.phone)) {
    errors.value.phone = 'رقم هاتف غير صحيح'
    return false
  }
...
const handleClientRegister = async () => {
  ...
  try {
    ...
    if (avatarPreview.value && avatarPreview.value.startsWith('data:image')) {
      avatarToSend = avatarPreview.value
    }
    ...
    const result = await authStore.registerCustomer({
      ...
      avatar: avatarToSend || defaultAvatarPath
    })
    ...
    if (result.success) {
      ...
      showNotification(result.error || 'حدث خطأ أثناء التسجيل', 'error')
    }
  } catch (error) {
...
const handleAvatarUpload = (event) => {
  ...
  if (file) {
    ...
    reader.onerror = () => showNotification('❌ فشل تحميل الصورة', 'error')
...
const getEnteredCode = () => {
  return codeDigits.value.join('').replace(/\D/g, '').trim()
}
...
const verifyCode = async () => {
  ...
  const expectedDevCode = String(devCode.value || '').trim()
  ...
  if (expectedDevCode && code === expectedDevCode) {
    forgotStep.value = 3
  ...
  try {
    ...
    if (expectedDevCode && code === expectedDevCode) {
      ...
      showNotification(error.response?.data?.message || 'رمز غير صحيح أو منتهي الصلاحية', 'error')
    } else {
...
const closeForgotModalAndLogin = () => {
  ...
  loginForm.password = ''
  switchView('login')
  showNotification('يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة', 'success')
}
...
// ===== INITIALISATION =====
// ===== CALLBACK OAUTH CORRIGÉ (dans onMounted) =====
onMounted(async () => {
  resetAllForms()
  ...
  if (token && userStr) {
    try {
      ...
      console.error('Erreur lors du traitement du retour OAuth:', e)
...
<style scoped>
/* ===== STYLES - Page login AVEC POLICE AMIRI ===== */
* {
  margin: 0;
  padding: 0;
...
.is-valid .form-input { border-color: #10b981; }
.error-message { color: #ef4444; font-size: 0.75rem; }
.success-message { color: #10b981; font-size: 0.75rem; }
...
.email-valid-icon, .email-invalid-icon {
  position: absolute;
  left: 12px;
  font-size: 1rem;
}
...
.password-input { padding-left: 45px; }
...
.toggle-password:hover { color: #d40025; }
...
.forgot-link:hover { color: #d40025; }
...
.step.active .step-number { background: #d40025; color: white; }
.step.completed .step-number { background: #10b981; color: white; }

.step-label { font-size: 0.7rem; color: #64748b; }
.step.active .step-label { color: #d40025; font-weight: 600; }
...
.step-line.active { background: #d40025; }

/* Photo Upload */
.photo-upload-section { text-align: center; margin: 20px 0; }
.avatar-upload-container { display: flex; flex-direction: column; align-items: center; gap: 12px; }
...
.upload-hint { color: #94a3b8; font-size: 0.75rem; }
.default-hint { color: #08717f; font-size: 0.7rem; margin-top: 4px; }

.skip-option { text-align: center; margin: 15px 0; }
.skip-btn { background: none; border: none; color: #64748b; font-size: 0.85rem; cursor: pointer; transition: color 0.3s ease; }
.skip-btn:hover { color: #d40025; }
...
.next-btn { background: linear-gradient(135deg, #d40025, #b00020); color: white; }
.next-btn:hover:not(:disabled) { transform: translateX(2px); }
...
.success-container h3 { font-size: 1.3rem; color: #1e293b; margin-bottom: 8px; }
.success-container p { color: #64748b; margin-bottom: 24px; }
...
.success-btn.primary { background: linear-gradient(135deg, #08717f, #d40025); color: white; }
.success-btn.primary:hover { transform: translateY(-2px); }
.success-btn.secondary { background: #f1f5f9; color: #64748b; }
.success-btn.secondary:hover { background: #e2e8f0; }
...
.create-account { background: linear-gradient(135deg, #08717f, #065a69); color: white; border: none; }
.create-account:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(8, 113, 127, 0.3); }

.artisan { display: flex; justify-content: space-between; align-items: center; background: #f8fafc; }
.btn-highlight { color: #d40025; font-weight: 700; }
...
.modal-close:hover { background: #d40025; color: white; transform: rotate(90deg); }

.modal-body { padding: 28px 24px; text-align: center; background: white; }
.modal-icon { font-size: 48px; margin-bottom: 16px; }
.modal-body h4 { font-size: 1.3rem; margin-bottom: 8px; color: #1e293b; font-weight: 700; }
.modal-body p { color: #64748b; margin-bottom: 24px; font-size: 0.9rem; }
...
@media (max-width: 768px) {
  ...
  .brand-column { padding: 32px 24px; }
  .form-column { padding: 32px 24px; }
  .social-login { flex-direction: column; }
  .action-buttons { flex-direction: column; }
  .brand-logo { width: 100px; }
  .toast-notification { right: 20px; left: 20px; bottom: 16px; }
}

@media (max-width: 480px) {
  .success-actions { flex-direction: column; }
  .code-inputs { gap: 6px; }
  .code-input { width: 35px; height: 40px; font-size: 1rem; }
}
</style>
